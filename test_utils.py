...
def test_stft_of_scipy_librosa():
    ...
    a = signal.stft(x, window='hamming', nperseg=nperseg, boundary=None, padded=False)[-1]
    b = librosa.core.stft(x, window='hamming', n_fft=nperseg, hop_length=nperseg//2, center=False)/(nperseg//2)
...
def test_mel():
    ...
    b = librosa.filters.mel(sr=sample_rate, n_fft=nperseg, n_mels=nmels, fmax=fmax, htk=True)
...
def test_mfcc():
    ...
    a = mfcc(data, sample_rate=sr, nmfcc=nmfcc, nperseg=nperseg, noverlap=noverlap, nmels=nmels)
...
def test_pre_emphasis():
    ...
    assert np.allclose(pre_emphasis(x, coef=0.9), librosa.effects.preemphasis(x, coef=0.9))
